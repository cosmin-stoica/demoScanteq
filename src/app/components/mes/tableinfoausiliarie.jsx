  ...
  const handleElimina = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios.delete('https://www.scanteq.com/php/infoausiliarie.php', { data: { idsToDelete: selectedRows } })
          .then((response) => {
  ...
  const handleSaveChanges = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios
          .put('https://www.scanteq.com/php/infoausiliarie.php', { modifiedData })
          .then((response) => {
    ...
    <div>
      {isUserAllowed ? (
        <div className='tablepaneavv'>
          <h1>Gestione Info Ausiliarie</h1>
  ...
  <p className='current'>{`Pagina ${currentPage + 1}/${Math.ceil(filteredRisultati.length / itemsPerPage)}`}</p>
    ...
    <td><button className='visualizzabutton' onClick={() => handleVisualizza(risultato.Immagine,risultato.Codice,risultato.Descrizione)}>Visualizza</button></td>
          ...
          <div className='infocomponentediv'>
            ...
            <img className='imagecomponente' src={selectedImmagine}></img>
          </div>
          </div>      
          )}
        </div>
...
export default TableInfoAusiliarie;
