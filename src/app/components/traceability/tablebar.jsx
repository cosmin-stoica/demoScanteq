...
function TableTracc() {
  ...
  axios.get('https://www.scanteq.com/php/barcode.php', {
    headers: {
  ...
  const filteredRisultati = Array.isArray(risultati) ? risultati.filter((risultato) => {
    ...
    const includesComponente = searchComponente === '' || (risultato.Componente && risultato.Componente.includes(searchComponente));

    return includesTimestamp && includesUser && includesOrder && includesWorkstation && includesCodice && includesSeriale && includesNomeJob && includesComponente;
}) : []
  ...
  const handleElimina = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        const custom = 'Sc38ahD38sW31Hshue';
        axios.delete('https://www.scanteq.com/php/barcode.php', { 
  ...
  const handleSaveChanges = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        const custom = 'Sc38ahD38sW31Hshue';
        axios
          .put('https://www.scanteq.com/php/barcode.php', { data: {modifiedData},headers: {
            'Custom-Header': custom,
          },  },
          )
          .then((response) => {
...
const handleAggiorna = () => {
  ...
  <p className='current'>{`Pagina ${currentPage + 1}/${Math.ceil(filteredRisultati.length / itemsPerPage)}`}</p>
        ...
        <input
          className='cercatext'
          type="text"
          placeholder="Cerca per componente"
          value={searchComponente}
          onChange={(e) => {
            setSearchComponente(e.target.value);
            handleFilterChange();
          }}
        />
            <button onClick={() => {
    ...
    {renderTableCell(risultato.ID, 'Componente', risultato.Componente)}
    {renderTableCell(risultato.ID, 'Descrizione', risultato.Descrizione)}
  </tr>
              ))}
            </tbody>
          </table>
