  ...
  const handleElimina = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios.delete('https://www.scanteq.com/php/coppia.php', { data: { idsToDelete: selectedRows } })
          .then((response) => {
  ...
  const handleSaveChanges = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios
          .put('https://www.scanteq.com/php/coppia.php', { modifiedData })
          .then((response) => {
...
const handleAggiorna = () => {
  ...
  <p className='current'>{`Pagina ${currentPage + 1}/${Math.ceil(filteredRisultati.length / itemsPerPage)}`}</p>
          ...
          <table className='tracctable'>          
            <thead>
              ...
              <th>Seleziona</th>
                ...
                <th>Nome Job</th>
                <th>Data e Ora</th>
                <th>Coppia Nominale</th>
                <th>Tolleranza Coppia</th>
                <th>Coppia Misurata</th>
                <th>Errore</th>
                <th>Ripetizioni</th>
                <th>Esito</th>
              </tr>
    ...
    {renderTableCell(risultato.ID, 'CoppiaNominale', risultato.CoppiaNominale)}
    {renderTableCell(risultato.ID, 'TolleranzaCoppia', risultato.TolleranzaCoppia)}
    {renderTableCell(risultato.ID, 'CoppiaMisurata', risultato.CoppiaMisurata)}
