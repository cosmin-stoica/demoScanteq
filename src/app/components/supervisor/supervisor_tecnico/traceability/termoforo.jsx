  ...
  const handleElimina = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios.delete('https://www.scanteq.com/php/termoforo.php', { data: { idsToDelete: selectedRows } })
          .then((response) => {
  ...
  const handleSaveChanges = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios
          .put('https://www.scanteq.com/php/termoforo.php', { modifiedData })
          .then((response) => {
...
const handleAggiorna = () => {
  ...
  <p className='current'>{`Pagina ${currentPage + 1}/${Math.ceil(filteredRisultati.length / itemsPerPage)}`}</p>
    ...
    {renderTableCell(risultato.ID, 'CorrenteNominale', risultato.CorrenteNominale)}
    {renderTableCell(risultato.ID, 'TolleranzaCorrente', risultato.TolleranzaCorrente)}
    {renderTableCell(risultato.ID, 'CorrenteMisurata', risultato.CorrenteMisurata)}
