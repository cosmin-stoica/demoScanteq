  ...
  const handleElimina = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios.delete('https://www.scanteq.com/php/variables.php', { data: { idsToDelete: selectedRows } })
          .then((response) => {
  ...
  const handleSaveChanges = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios
          .put('https://www.scanteq.com/php/variables.php', { modifiedData })
          .then((response) => {
...
const handleAggiorna = () => {
  ...
  <p className='current'>{`Pagina ${currentPage + 1}/${Math.ceil(filteredRisultati.length / itemsPerPage)}`}</p>
    ...
    {renderTableCell(risultato.ID, 'IdJob', risultato.IdJob)}
    {renderTableCell(risultato.ID, 'NomeJob', risultato.NomeJob)}
    {renderTableCell(risultato.ID, 'Timestamp', risultato.Timestamp)}
    {renderTableCell(risultato.ID, 'IdVite', risultato.IdVite)}
    {renderTableCell(risultato.ID, 'GP', risultato.GP)}
    {renderTableCell(risultato.ID, 'Bussola', risultato.Bussola)}
    {renderTableCell(risultato.ID, 'CoppiaMin', risultato.CoppiaMin)}
    {renderTableCell(risultato.ID, 'CoppiaMax', risultato.CoppiaMax)}
    {renderTableCell(risultato.ID, 'CoppiaFinale', risultato.CoppiaFinale)}
    {renderTableCell(risultato.ID, 'AngoloMin', risultato.AngoloMin)}
    {renderTableCell(risultato.ID, 'AngoloMax', risultato.AngoloMax)}
    {renderTableCell(risultato.ID, 'AngoloFinale', risultato.AngoloFinale)}
    {renderTableCell(risultato.ID, 'Ripetizioni', risultato.Ripetizioni)}
