...
function CreaPrn() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const imageRef = ref(storage, `imagesprn/${immagine.name}`);
      await uploadBytes(imageRef, immagine);

      const imageUrl = await getDownloadURL(imageRef);

      const response = await axios.post('https://www.scanteq.com/php/componenti.php', {
        Codice: codice,
