...
function TableLavorazioni() {
  ...
  useEffect(() => {
    axios.get('https://www.scanteq.com/php/lavorazioni.php')
    .then((response) => {
  ...
  const filteredRisultati = Array.isArray(risultati) ? risultati.filter((risultato) => {
    const includesCommessa = searchCommessa === '' || (risultato.Commessa && risultato.Commessa.includes(searchCommessa));

    return includesCommessa;
}) : []
  ...
  const handleElimina = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios.delete('https://www.scanteq.com/php/lavorazioni.php', { data: { idsToDelete: selectedRows } })
          .then((response) => {
  ...
  const handleSaveChanges = () => {
    if (selectedRows.length > 0) {
      ...
      if (userConfirmed) {
        ...
        axios
          .put('https://www.scanteq.com/php/lavorazioni.php', { modifiedData })
          .then((response) => {
...
const handleAggiorna = () => {
  ...
  <p className='current'>{`Pagina ${currentPage + 1}/${Math.ceil(filteredRisultati.length / itemsPerPage)}`}</p>
          ...
          onChange={(e) => {
            setSearchCommessa(e.target.value);
            handleFilterChange();
          }}
        />
    ...
    {renderTableCell(risultato.ID, 'Data', risultato.Timestamp)}
  </tr>
              ))}
            </tbody>
          </table>
...
export default TableLavorazioni;
