...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    ...
    if (isset($data['Commessa'], $data['Codice'], $data['Seriale'], $data['Postazione'], $data['Utente'], $data['NomeJob'], $data['IdJob'], $data['Timestamp'], $data['CorrenteNominale'], $data['TolleranzaCorrente'], $data['CorrenteMisurata'], $data['Errore'], $data['Ripetizioni'], $data['Esito'])) {
        // Esegui le operazioni specifiche per le richieste HTTP POST qui
        // Inserisci i dati nella tabella "avvitature"
        $query = "INSERT INTO termoforo (Commessa, Codice, Seriale, Postazione, Utente, NomeJob, IdJob, Timestamp, CorrenteNominale, TolleranzaCorrente, CorrenteMisurata, Errore, Ripetizioni, Esito  ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($query);

        if ($stmt) {
            $stmt->bind_param("ssisssisdddsis", $data['Commessa'], $data['Codice'], $data['Seriale'], $data['Postazione'], $data['Utente'], $data['NomeJob'], $data['IdJob'], $data['Timestamp'], $data['CorrenteNominale'], $data['TolleranzaCorrente'], $data['CorrenteMisurata'], $data['Errore'], $data['Ripetizioni'], $data['Esito']);
            if ($stmt->execute()) {
    ...
    if (!empty($idsToDelete)) {
        ...
        $stmt = $db->prepare("DELETE FROM termoforo WHERE ID IN ($placeholders)");
    ...
    foreach ($modifiedData as $row) {
        ...
        $correnteNominale = $row['CorrenteNominale'];
        $tolleranzaCorrente = $row['TolleranzaCorrente'];
        $correnteMisurata = $row['CorrenteMisurata'];
        $errore = $row['Errore'];
        $ripetizioni = $row['Ripetizioni'];
        $esito = $row['Esito'];

        // Implementa la logica per aggiornare i dati nel database
        $stmt = $db->prepare("UPDATE termoforo SET Commessa=?, Codice=?, Seriale=?, Postazione=?, Utente=?, IdJob=?, NomeJob=?, Timestamp=?, CorrenteNominale=?, TolleranzaCorrente=?, CorrenteMisurata=?, Errore=?, Ripetizioni=?, Esito=? WHERE ID=?");
    ...
    $dataFromHeader) {
        ...
        $query .= "CorrenteMisurata AS ValoreFinale FROM termoforo";
