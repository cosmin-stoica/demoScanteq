...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    ...
    if (isset($data['Postazione'], $data['Codice'], $data['PRN'])) {
        $query = "INSERT INTO lavorazioni (Postazione, Commessa) VALUES (?, ?)";
        $stmt = $db->prepare($query);
        
        if ($stmt) {          
            $stmt->bind_param("ss", $data['Postazione'], $data['Commessa']);
            if ($stmt->execute()) {
    ...
    if (!empty($idsToDelete)) {
        ...
        $stmt = $db->prepare("DELETE FROM lavorazioni WHERE ID IN ($placeholders)");
    ...
    foreach ($modifiedData as $row) {
        ...
        $stmt = $db->prepare("UPDATE lavorazioni SET Postazione=?, Commessa=? WHERE ID=?");
        $stmt->bind_param('ssi', $postazione, $commessa, $id);
