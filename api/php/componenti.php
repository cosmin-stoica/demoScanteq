...
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    ...
    if (isset($data['Codice'], $data['Descrizione'], $data['Immagine'])) {
        $query = "INSERT INTO prn (Codice, Descrizione, Immagine) VALUES (?, ?, ?)";
    ...
    if (!empty($idsToDelete)) {
        ...
        $stmt = $db->prepare("DELETE FROM prn WHERE ID IN ($placeholders)");
    ...
    foreach ($modifiedData as $row) {
        ...
        $stmt = $db->prepare("UPDATE prn SET Codice=?, Descrizione=?, Immagine=? WHERE ID=?");
